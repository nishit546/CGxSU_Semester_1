...
- The **eigenspace** of an eigenvalue \(\lambda\) is the set of all eigenvectors corresponding to \(\lambda\) (including all scalar multiples).  
- For the above 3×3 example:  
\(\lambda = 0\) eigenspace is spanned by \([-1,1,0]\) and \([-1,0,1]\)  
...
\[
X = 
\begin{bmatrix}1 & -1 & -1 \\ 1 & 1 & 0 \\ 1 & 0 & 1\end{bmatrix}, \quad
D = 
